<div align="center">
  <h1>Olá, eu sou o Luca Almeida Lucareli </h1>
  <p><strong>Backend Developer | Performance Enthusiast | Software Architect</strong></p>

  <p>
    <a href="https://www.linkedin.com/in/lucalucareli">
      <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" />
    </a>
  </p>
</div>

---

### Sobre Mim

Sou um desenvolvedor focado em construir sistemas **escaláveis e de alta performance**. Minha abordagem combina os princípios de **Clean Architecture** com uma busca constante por eficiência computacional.

* **Sistemas Distribuídos:** Experiência na construção de APIs robustas e microsserviços.
* **Performance First:** Focado em otimização de queries, concorrência e baixo uso de recursos.
* **Tooling:** Usuário assíduo de **Neovim** (e amante de automações via CLI).
* **Data Science:** Conhecimento aplicado em algoritmos de Machine Learning para solução de problemas complexos.

---

### Tech Stack

<div align="center">
  <img src="https://skillicons.dev/icons?i=go,rust,ts,python,java,lua,nodejs,nest,npm,prisma,docker,gcp,aws,azure,terraform,linux,git,vim,neovim,bash&perline=10" />
</div>

#### Databases & Storage
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-336791?style=flat-square&logo=postgresql&logoColor=white)
![SQL Server](https://img.shields.io/badge/SQL%20Server-CC2927?style=flat-square&logo=microsoft-sql-server&logoColor=white)
![MongoDB](https://img.shields.io/badge/MongoDB-47A030?style=flat-square&logo=mongodb&logoColor=white)
![Redis](https://img.shields.io/badge/Redis-D82C20?style=flat-square&logo=redis&logoColor=white)
![Neo4j](https://img.shields.io/badge/Neo4j-018BFF?style=flat-square&logo=neo4j&logoColor=white)

#### Machine Learning & Intelligence
![Decision Trees](https://img.shields.io/badge/Decision%20Trees-8BC34A?style=for-the-badge)
![Neural Networks](https://img.shields.io/badge/Neural%20Networks-FF6F00?style=for-the-badge)
![Naive Bayes](https://img.shields.io/badge/Naive%20Bayes-9C27B0?style=for-the-badge)
![KNN](https://img.shields.io/badge/KNN-3F51B5?style=for-the-badge)
![KMeans](https://img.shields.io/badge/KMeans-009688?style=for-the-badge)
![Linear Regression](https://img.shields.io/badge/Linear%20Regression-795548?style=for-the-badge)
![Chatbots](https://img.shields.io/badge/Chatbots-00BCD4?style=for-the-badge)
 

---

### GitHub Stats & Activity

<div align="center">
  <img height="180em" src="https://github-readme-stats-sigma-five.vercel.app/api?username=LucaLucareli&show_icons=true&theme=tokyonight&hide_border=true&count_private=true&include_all_commits=true" />
  <img height="180em" src="https://github-readme-stats-sigma-five.vercel.app/api/top-langs/?username=LucaLucareli&layout=compact&theme=tokyonight&hide_border=true" />
</div>

![Activity Graph](https://github-readme-activity-graph.vercel.app/graph?username=LucaLucareli&theme=react-dark)

---

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/LucaLucareli/LucaLucareli/output/dist/pacman-contribution-graph-dark.svg">
  <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/LucaLucareli/LucaLucareli/output/dist/pacman-contribution-graph.svg">
  <img alt="pacman contribution graph" src="https://raw.githubusercontent.com/LucaLucareli/LucaLucareli/output/pacman-contribution-graph.svg">
</picture>
